...
# --- THÊM VÀO CUỐI FILE byte.py ---

def create_udp_handshake(uid, token):
    """
    Tạo gói tin bắt tay UDP (Handshake).
    Cấu trúc thường là: [Header 1 byte] + [Độ dài Token] + [Token] + [UID]
    Bạn cần bắt gói tin thực tế để biết Header là gì (ví dụ: 0x81, 0x01, v.v.)
    """
    try:
        # --- PHẦN CẦN SỬA ---
        # Đây là ví dụ giả định. Bạn phải thay '81' bằng header thực tế bạn sniff được.
        header = "81" 
        
        # Token thường được gửi dạng bytes
        ...
        # UID chuyển sang hex (Little Endian hoặc Big Endian tùy game)
        # Ví dụ này dùng Big Endian
        uid_hex = hex(int(uid))[2:]
        if len(uid_hex) % 2 != 0:
            uid_hex = "0" + uid_hex
            
        # Ghép gói tin: Header + Độ dài Token (1 byte) + Token + UID
        # Lưu ý: Cấu trúc này thay đổi theo phiên bản game
        packet_hex = f"{header}{dec_to_hex(token_len)}{token_hex}{uid_hex}"
        
        return bytes.fromhex(packet_hex)
    except Exception as e:
        print(f"Error creating UDP handshake: {e}")
        return None

def create_udp_keepalive(uid):
    """
    Tạo gói tin duy trì kết nối UDP (Ping).
    Gửi mỗi 1-2 giây để server biết bạn chưa 'out'.
    """
    try:
        # --- PHẦN CẦN SỬA ---
        # Header ping UDP (ví dụ: 0x00 hoặc 0x02)
        # Đôi khi ping chỉ là 1 byte rỗng hoặc chứa timestamp
        header = "00" 
        
        # Có thể cần kèm UID hoặc Sequence Number
        return bytes.fromhex(header)
    except Exception as e:
        print(f"Error creating UDP keepalive: {e}")
        return b'\x00' # Gửi byte rỗng nếu lỗi
