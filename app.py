...
class FF_CLIENT(threading.Thread):
    ...
    def api_stop_bot(self):
        ...
        msg = f"Đã dừng Auto Start team {tc}"
        log_message(msg)
        return msg
    ...
    def get_tok(self):
        ...
        if not res:
            log_message("Login thất bại. Kiểm tra ID/Pass.")
            return
        ...
        zeros = "00000000" if length == 8 else "0000000"
    ...
    def sock_online(self, token, ip, port):
        while True:
            try:
                ...
                log_message(f"Connected Online (Lobby): {ip}:{port}")
                
                # Bật Heartbeat TCP
                threading.Thread(target=self.keep_alive_tcp, args=(self.socket_client,), daemon=True).start()
                ...
                while True:
                    ...
                    try:
                        # Decode packet bằng protobuf
                        json_str = get_available_room(data.hex())
                        if json_str:
                            ...
                            # TỰ ĐỘNG QUÉT DỮ LIỆU ĐỂ TÌM IP VÀ PORT
                            for key, val in parsed.items():
                                if isinstance(val, dict) and "data" in val:
                                    content = val["data"]
                                    if isinstance(content, str) and content.count('.') == 3:
                                        if not content.startswith("192.168") and not content.startswith("10."):
                                            game_ip = content
                                    elif isinstance(content, int) and 10000 < content < 65535:
                                        game_port = content
                            
                            if game_ip and game_port:
                                log_message(f"[AUTO] TÌM THẤY TRẬN! IP: {game_ip}:{game_port}")
                                match_token = str(self.id) 
                                if self.udp_client: self.udp_client.stop()
                                self.udp_client = GameClientUDP(game_ip, game_port, match_token, self.id)
    ...
    def sock_whisper(self, token, ip, port):
        while True:
            try:
                ...
                # Bật Heartbeat cho Whisper
                threading.Thread(target=self.keep_alive_tcp, args=(self.clients,), daemon=True).start()
...
def api_help():
    help_text = (
        "<b>FF AUTO START BOT (UDP SUPPORT)</b><br><br>"
        "<b>Commands:</b><br>"
...
def api_start(team_code):
    if not bot_instance:
        return jsonify({"status": "error", "message": "Bot chưa khởi tạo (kiểm tra bot.txt)"})
    
    if not team_code.isdigit():
        return jsonify({"status": "error", "message": "Team code phải là số."})
...
def api_stop():
    if not bot_instance:
        return jsonify({"status": "error", "message": "Bot chưa khởi tạo."})
...
def start_bot_background():
    ...
    try:
        if not os.path.exists("bot.txt"):
            log_message("Error: Không tìm thấy file bot.txt!")
            return
        ...
        if not data:
            log_message("Error: bot.txt rỗng!")
            return

        uid, pwd = list(data.items())[0]
        log_message(f"Khởi động bot với UID: {uid}")
        bot_instance = FF_CLIENT(uid, pwd)
    except Exception as e:
        log_message(f"Khởi tạo bot thất bại: {e}")
